...
def movePlayer1(steps):
    ...
    if (boxPosition):
        ...
        if (diceValue == remainingSteps):
            ...
            global finishingBox, SERVER, playerName
...
def movePlayer2(steps):
    ...
    t = rightBoxes[-2::-1]
    boxPosition = checkColorPosition(t,"red")
...
def handleWin(message):
    global playerType, rollButton, canvas2, winingMessage, screen_height, screen_width, resetButton
    if ('Red' in message):
        if(playerType == "player2"):
            rollButton.destroy()
    ...
    resetButton.place(x = screen_width/2, y= screen_height/2 - 80)
    
def updateScore(message):
    global canvas2, player1Score, player2Score, player2ScoreLabel, player1ScoreLabel
    if ('Red' in message):
        player1Score +=1
    if ('Yellow' in message):
        player2Score +=1
